# ================== MAPA DE PACIENTES POR NIVEL DE SEVERIDAD (SOLO PUERTO MONTT) ==================
...
# -------------------------------------------------------------------
# ---- 4. Graficar (solo Puerto Montt) ----
fig, ax = plt.subplots(1, 1, figsize=(8, 10))

# 4.1 Pintar SOLO la comuna de Puerto Montt
...
# 4.2 Puntos por cada nivel de severidad (solo dentro de Pto. Montt)
handles = []
labels_leg = []

for sev_var in SEV_VARS:
    if sev_var not in pts_pm.columns:
        continue

    # Presencia si valor > 0 / True
    submask = pd.to_numeric(pts_pm[sev_var], errors='coerce').fillna(0) > 0
    if not submask.any():
        continue
...
# 4.3 Hospital de Puerto Montt (estrella)
hpm_handle = None
...
plt.tight_layout()
plt.savefig("severidad21.svg", format="svg")
plt.show()
