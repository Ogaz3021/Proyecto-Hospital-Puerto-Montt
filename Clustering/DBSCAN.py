# ================== DBSCAN (variables solicitadas) + DUNN ==================
...
EPS = 0.124        # ajusta según escala de X (tras preprocesamiento)
MIN_SAMPLES = 6
RANDOM_STATE = 42  # (no afecta DBSCAN, pero lo dejamos para coherencia)
...
def filtrar_llanquihue(df, target_prov=None, tol_m=250):
    ...
    - Detecta la columna de nombre de comuna (para colorear el mapa).
    """
    ...
    # Fallback por cercanía en metros para puntos no asignados
...
# ---- Features para clustering (solo variables solicitadas) ----
present = [v for v in CLUSTER_VARS if v in gdf_ll.columns]
if len(present) < 3:
    raise ValueError(f"Variables disponibles insuficientes para DBSCAN: {present}")
...
# ---- DBSCAN (Bray–Curtis) ----
db = DBSCAN(eps=EPS, min_samples=MIN_SAMPLES, metric='braycurtis')
labels = db.fit_predict(X)

# ================== SALIDAS SOLICITADAS ==================
print("\n=== RESULTADOS DBSCAN (variables solicitadas) ===")

# (1) Número de clusters encontrados (excluye ruido -1)
...
# (2) Métricas internas en puntos NO-RUIDO
if valid_labels.size > 1 and n_clusters >= 2:
    mask_valid = labels != -1
    X_valid = X[mask_valid]
    lab_valid = labels[mask_valid]

    sil = silhouette_score(X_valid, lab_valid, metric='braycurtis')
    dbi = davies_bouldin_score(X_valid, lab_valid)
    ch  = calinski_harabasz_score(X_valid, lab_valid)

    # Dunn con distancias Bray–Curtis completas
    D_all   = squareform(pdist(X, metric='braycurtis'))
    idx_val = np.where(mask_valid)[0]
    D_valid = D_all[np.ix_(idx_val, idx_val)]
    ...
    _dunn = 'inf' if np.isinf(dunn) else ('nan' if np.isnan(dunn) else f'{dunn:.3f}')
    print(f"   Dunn (más alto mejor): {_dunn}")
else:
    print("2) Silhouette / DB / CH / Dunn: no aplican (clusters insuficientes tras excluir ruido)")

# (3) Tablas resumen por cluster
df_out = df_feat.copy()
df_out['cluster_dbscan'] = labels

# Añadimos "tiempo (minutos)" y "km" para reportes descriptivos si existen
...
sizes_all = df_out['cluster_dbscan'].value_counts().sort_index()
for k_, v_ in sizes_all.items():
    tag = " (ruido)" if k_ == -1 else ""
    print(f"  - Etiqueta {k_}{tag}: {v_} filas")

df_valid = df_out[df_out['cluster_dbscan'] != -1].copy()
if df_valid.empty or n_clusters == 0:
    print("\n[Resumen] No hay clusters válidos (todo fue ruido).")
else:
    # asegurar numérico
    num_all = [c for c in df_valid.columns if c not in ['cluster_dbscan', 'Sexo (Desc)']]
    for c in num_all:
        df_valid[c] = pd.to_numeric(df_valid[c], errors='coerce')

    means = df_valid.groupby('cluster_dbscan')[num_all].mean().round(4)
    ...
    agg_std = df_valid.groupby('cluster_dbscan')[num_all].std(ddof=0).round(4)
    agg_iqr = df_valid.groupby('cluster_dbscan')[num_all].agg(iqr).round(4)
    agg_cv  = df_valid.groupby('cluster_dbscan')[num_all].agg(cv).round(4)
    ...
    for k_ in sorted(df_valid['cluster_dbscan'].unique()):
        sub = df_valid[df_valid['cluster_dbscan']==k_][num_all]
...
# (4) MAPA geográfico con relleno por comuna + puntos translúcidos
if all(c in gdf_ll.columns for c in ['lat','lng']):
    gmap = gdf_ll.copy()
    gmap['cluster_dbscan'] = labels

    # Colores base para clusters válidos (excluye ruido -1)
    uniq = np.sort(np.unique(labels[labels!=-1]))  # solo clusters, sin ruido
    cmap = plt.cm.get_cmap('tab20', max(len(uniq), 3))
    colors_pts = {c: cmap(i % 20) for i, c in enumerate(uniq)}

    # Diccionario de colores por comuna (relleno del polígono)
    comuna_colors = {
        "puerto montt": "#fdecf5",
        "puerto varas": "#efefef",
        "calbuco": "#fadcdd",
    ...
    # Normalizar nombre comuna para hacer match con las llaves del diccionario
    def norm_name(x):
    ...
    # 4.1 Dibujar comunas con el color correspondiente
    ...
    # 4.2 Borde de provincia más grueso
    prov_ll.boundary.plot(ax=ax, linewidth=1.5, color='black')

    # 4.3 Puntos de ruido (-1), en negro con 'x'
    #    OJO: aquí el alpha es alto (0.9) para que NO sea afectado por la opacidad reducida.
    if (gmap['cluster_dbscan']==-1).any():
        gmap[gmap['cluster_dbscan']==-1].plot(
            ax=ax,
            color='black',
            markersize=22,
            alpha=1,        # <- ruido casi opaco
            marker='x',
            edgecolor='black',
            linewidth=0.3,
            label='Ruido (-1)',
    ...
    # 4.4 Puntos de clusters válidos (alpha más baja para ver sobreposición)
    for c in uniq:
        msk = gmap['cluster_dbscan'] == c
        gmap[msk].plot(
            ax=ax,
            color=colors_pts[c],
            markersize=28,
            alpha=0.45,       # <- clusters translúcidos
            marker='o',
            edgecolor='black',
            linewidth=0.3,
            label=f'Cluster {c}',
    ...
    # 4.5 Centroides geográficos de cada cluster válido (sin ruido)
    if len(uniq) > 0:
        cent = gmap[gmap['cluster_dbscan']!=-1].groupby('cluster_dbscan')[['lng','lat']].mean()
        ax.scatter(
            cent['lng'],
            cent['lat'],
            s=260,
    ...
    ax.set_title('Mapa: DBSCAN (Provincia de Llanquihue)')
    ax.set_axis_off()
    ax.legend(
        loc='lower left',
        frameon=True,
